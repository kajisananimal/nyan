...
function prettyCompatibility(a, b){
  if (a===b) return {grade:"◎", msg:"同タイプ同士！価値観が合いやすく最強ペア。"};
  const bestA = TYPES[a]?.best || [];
  const bestB = TYPES[b]?.best || [];
  const aLikesB = bestA.includes(b);
  const bLikesA = bestB.includes(a);
  if (aLikesB && bLikesA) return {grade:"◎", msg:"お互いの強みを補完できる“相互ベスト”相性！"};
  if (aLikesB || bLikesA) return {grade:"○", msg:"片方の得意がもう片方の安心に。伸びる相性。"};
  return {grade:"△", msg:"悪くないけど工夫が必要。違いをルール化すると安定。"};
}
